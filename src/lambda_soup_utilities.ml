...
let get_first_item_of_all_unordered_lists contents : string list =
  ...
  $$ "ul"
  |> to_list
  |> List.map ~f:(fun x -> x $ "li" |> texts |> String.concat ~sep:"" |> String.strip)
;;
...
let get_first_item_of_second_unordered_list contents : string =
  let first_elements = get_first_item_of_all_unordered_lists contents in
  List.nth_exn first_elements 1 (* index 1 is second element *)
;;
